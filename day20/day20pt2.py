...
class ConjunctionModule(Module):
    def receivePulse(self, pulse:bool, sendingModule: 'Module'):
        ...
        self.nextPulse.put((outPulse, sendingModule))
...
vrInputModules =  {module.id: None for module in Module.modulesDict['vr'].inputModules}

buttonPresses = 0
while None in vrInputModules.values():
    buttonPresses +=1
    Module.pressButton()
    while not Module.pulseQue.empty():
        module = Module.pulseQue.get()
        isVr = module.sendPulse()
        if isVr:
            vrInputModules[isVr] = buttonPresses

print(math.lcm(*vrInputModules.values()))
